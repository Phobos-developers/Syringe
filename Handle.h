...
struct Handle
{
    ...
    explicit Handle(value_type value) noexcept : Value(value)
    {
    }
...
struct VirtualMemoryHandle
{
    ...
    VirtualMemoryHandle(HANDLE process, LPVOID address, SIZE_T size) noexcept : Process(process)
    {
        if (process && size)
        {
    ...
    VirtualMemoryHandle(LPVOID allocated, HANDLE process) noexcept : Value(allocated), Process(process)
    {
    }
    ...
    VirtualMemoryHandle(VirtualMemoryHandle&& other) noexcept
        : Value(std::exchange(other.Value, nullptr)), Process(std::exchange(other.Process, nullptr))
    {
    }
